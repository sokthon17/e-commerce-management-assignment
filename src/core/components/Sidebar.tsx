        ...
        {SIDEBAR_MENU.map((list, idx) => {
          ...
          return (
            <Link
              key={idx}
              href={list.route || ''}
              className="relative flex h-[72px] cursor-pointer items-center gap-3 pl-[45px] transition-all ease-in-out hover:bg-blue-800"
            >
              <span
                className={clsx(
                  isActive ? 'left-0' : '-left-1',
                  'absolute top-0 h-full w-1 rounded-tr-full rounded-br-full bg-white transition-all ease-in-out'
                )}
              ></span>
              <Image
